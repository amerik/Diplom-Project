...
class JanusCall {
  ...
  */
  ...
  async registerUsername(username, user){
  ...
  onLocalStream(stream) { 
    Janus.attachMediaStream($('#my_video').get(0), stream);
    this.stream = stream;
    $("#my_video").get(0).muted = "muted";
  ...
  async initSession() {
    return new Promise((resolve, reject) => {
      this.janus = new Janus({
        server: "http://" +document.location.hostname +":8088/janus",
        success: resolve,
        error: reject,
        destroyed: function() {
...
var start_call = document.querySelector('#start_call');
var answer_call = document.querySelector('#answer_call');
var cancel_call = document.querySelector('#cancel_call');
