...
# bundle exec rake pit:first_and_last

namespace :pit do
  desc "Create all home order"
  task :first_and_last => :environment do

    ActiveRecord::Base.connection.execute("CREATE OR REPLACE FUNCTION first_element_state(anyarray, anyelement)
			  RETURNS anyarray AS
			$$
